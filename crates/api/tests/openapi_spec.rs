...
fn openapi_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("src/openapi.json")
}

fn read_openapi() -> serde_json::Value {
    let text = fs::read_to_string(openapi_path()).expect("read api openapi.json");
    serde_json::from_str(&text).expect("parse api openapi.json")
}
...
#[test]
fn api_openapi_is_valid_json_object() {
